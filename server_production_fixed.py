#!/usr/bin/env python3
"""
CrisisSim MCP Server - Production Version (Fixed)
Optimized for deployment with environment variables and production features.
"""
...
Make it realistic and engaging for crisis simulation training."""
            ...
            logger.info("✅ Generated scenario using OpenAI API")
            
        except Exception as e:
            # Fall back to template generation
            scenario = generate_template_scenario(crisis_type, location, people_count)
            logger.warning(f"⚠️ OpenAI API failed, using template generation: {str(e)}")
    else:
        # No API key available, use template
        scenario = generate_template_scenario(crisis_type, location, people_count)
        logger.info("ℹ️ No OpenAI API key available, using template generation")
...
Keep it realistic and maintain the same level of detail (4-6 sentences)."""
            ...
            logger.info("✅ Generated next step using OpenAI API")
...
async def main():
    ...
    logger.info("Starting CrisisSim MCP Server (Production Version - Fixed)")
    logger.info(f"OpenAI API Key available: {'Yes' if OPENAI_API_KEY else 'No'}")
